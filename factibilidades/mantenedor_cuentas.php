<?php
	...
	$objGrid = new datagrid('mantenedor_cuentas.php','1');
	...
	#es: titulo
	$objGrid-> tituloGrid("Mantenedor de usurarios del Sistema de TNA Solutions");
...
//	$objGrid->exportMagma = "_magma_exporta.php";
	...
	$objGrid->exportDetails['detalles'] = array("sql"=>"SELECT * FROM `enlaces_detalle` WHERE fac = '['fac']' ORDER BY `tipo` ASC",
										"parameters"=>"fac",
										"menu"=>"detalles");	
...
//////////////////////////////////////////////////////////////////////////
	...
	$objGrid -> FormatColumn("nombre", "Nombre", 60, 60, 0,"60", "left");
	$objGrid -> FormatColumn("apellido", "Apellido", 60, 60, 0,"60", "left");
	$objGrid -> FormatColumn("email", "Email", 60, 60, 0,"60", "left");
	$objGrid -> FormatColumn("empresa", "Empresa", 60, 60, 0,"300", "left");
	$objGrid -> FormatColumn("username", "User Name", 60, 60, 0,"60", "left");
	$objGrid -> FormatColumn("password", "Password", 60, 60, 0,"60", "left");
	$objGrid -> FormatColumn("token", "Token", 60, 60, 0,"20", "left");
...
//2.- Una función que evalué el arreglo de fechas obtenido, que contenga los feriados nacionales que correspondan (restando) y que reste los sábados y domingos. 
	...
	function fnext_fac() { // recupera la última fac utilizada y le suma 1
		...
		$conn = new mysqli('localhost', 'pitchi_script', 'P3rf3ct0.,', 'pitchi_enlaces');
		// Checkea conexion
		if ($conn->connect_error) die("Error de Conexion: " . $conn->connect_error);
		$sql = "SELECT `fac` FROM `enlaces_cabecera` order by `fac`DESC LIMIT 1";
