<?php
	...
	$objGrid = new datagrid('intercept_data_sample_II_grid.php','1');
	...
	if ($objGrid->isAjaxRequest()){
		switch ($objGrid->getAjaxID()){
			case DG_IsDelete: // case 3:	// Delete Rowa / Borrar Registro
				...
				#es: Get record ID
				$row = $objGrid->dgrtd;
				#es: Verificar si el registro tiene registros asociados
				#en: Verify if record has other related records
				$strSQL = sprintf("select * from payment_history where emp_id=%s", magic_quote($row));
				$arrData = $objGrid -> SQL_query($strSQL);
				
				#es: Si la consulta obtiene algún resultado, cancelar el borrado
				#en: If the query gets any records, cancel the deletion
				if (!empty($arrData)){
					#es: Definir la accion a ejecutar como 0 (nulo)
					#en: Define action as 0 (void)
					$objGrid->setAjaxID(0);
					
					#es: Mostrar una alerta de error
					#en: Display an error alert
					echo "<script>alert('The employee have payments, please delete payments first \\n\\n El empleado posee pagos, por favor elimine los pagos primero')</script>";
				}
			break;
		}
	}
