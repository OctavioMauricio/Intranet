<?php
	...
	$titulo = "<span id='titulo'>
				Factibilidades en Instalación <span id='fecha_hora'>".date("d/m/Y h:i:s")."  Refresh: ".
    ...
    $sql = "SELECT *  FROM `cabecera` WHERE (`estado` LIKE '05 instalacion' OR `estado` LIKE '06 en provision' OR `estado` LIKE '04 solicitar instalacion')";
  	$objGrid-> sqlstatement ($sql);
	#es: Definir campo para ordenamiento
	$objGrid-> orderby ("estado","desc");
	#es: Definir la cantidad de registros a mostrar por pagina
	$objGrid-> datarows(1000);
	#es: Definir una altura fija para el DataGrid
	#en: Define a fixed height for the DataGrid
	$objGrid-> height="700px";
...
//	$objGrid-> FormatColumn("id","ID", "40", "50", "2", "93", "right", "text");
	$objGrid-> FormatColumn("dummy_field_1","Detalle", "5", "30", 4, "40", "center", "imagelink:images/selected_rows.gif:viewDetails(%s),fac");
	$objGrid -> FormatColumn("fac", "FAC", 6, 6, 0, "20", "center", "number",$next_fact);
	$objGrid -> FormatColumn("estado", "Estado",  100, 100, 0, "100", "left", "select:00 sin solicitar:01 solicitado:02 cotizado proveedor:02 cotizado cliente:03 aprobado por cliente:04 solicitar instalacion:05 instalacion:06 en provision:07 en produccion:08 dado de baja:09 sin factibilidad:10 rechazada cliente:11 cliente rechazado:12 trasladado","00 sin solicitar");
	$objGrid -> FormatColumn("rut", "R.U.T.", 20, 20, 0,"80", "left");	
	$objGrid -> FormatColumn("nombre", "Nombre Completo", 150, 150, 0, "180", "left");
	$objGrid -> FormatColumn("direccion", "Direccion", 80, 80, 0,"200", "left");
	$objGrid -> FormatColumn("contacto", "Contacto", 100, 200, 0,"200", "left");	
	...
	$objGrid -> addRowStyle ("['estado']=='04 solicitar instalacion'", "colormagenta");
