...
// kickoff/includes/cm_cobranza_comercia_include.php
// Descripción: Genera tabla para el módulo de Cobranza Comercial.
// Autor: Mauricio Araneda
// Fecha: 2025-11-20 (Actualizado 2025-12-03)
...
// ----------------------------------------------------------
// Conexión a base Sweet
// ----------------------------------------------------------
$conn = DbConnect($db_sweet);
if (!$conn) {
    die("<b>Error:</b> No se pudo conectar a la base Sweet.");
}
$conn->set_charset('utf8mb4');


// ==========================================================
// 1) LISTA DINAMICA DE ESTADOS DESDE SWEET
...
$sql = "SELECT 
    ...
    LEFT JOIN icontel_clientes.cron_duemint_clients AS d_cli 
        ON d_cli.taxId = REPLACE(REPLACE(TRIM(acc.rut_c), '.', ''), ' ', '')
...
if (!$result) {
    die("<b>Error SQL:</b> " . $conn->error . "<br><pre>" . $sql . "</pre>");
}
...
while ($row = $result->fetch_assoc()) {
    ...
    $contenido .= "<tr class='{$clase}'>
        ...
        <td style='text-align:left;'>
            " . ($row["url_duemint"] ? 
                "<a href='" . htmlspecialchars($row["url_duemint"]) . "' target='_blank' title='Ver en Duemint' style='text-decoration:none;'>🔗</a>" 
                : "") . "
            <a target='_blank' 
...
<td style='text-align:center;'>
    " . (
        !empty($row["url_duemint"])
        ? "<a href='" . htmlspecialchars($row["url_duemint"]) . "' 
             target='_blank' 
             style='text-decoration:none; color:inherit;'>
                " . intval($row["num_doc_duemint"]) . "
           </a>"
        : intval($row["num_doc_duemint"])
    ) . "
</td>
        <td style='text-align:center;'>" . intval($row["dias_duemint"]) . "</td>

        <td>" . htmlspecialchars($row["ejecutivo"], ENT_QUOTES, 'UTF-8') . "</td>

        <td style='text-align:center;'>" . htmlspecialchars($row["fecha_modif"]) . "</td>
        <td style='text-align:center;'>" . intval($row["dias"]) . "</td>
