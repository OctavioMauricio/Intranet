...
<title>(<?php //echo $server; ?>)CDR iConTels SpA</title>
...
<body>
	<div class="wrapper"> <!-- comienzo de wrapper -->
		<form name="formulario" id="formulario" method="post" action="" class='no-print' >
		 <table>
		   <tr>
			...
			<?php
				...
				if(isset($_POST['id800'])) $_SESSION['id800'] = $_POST['id800'];
				...
				$sql = "SELECT * 
						FROM tnasolutions.did800 as did  INNER JOIN tnasolutions.clientes as cli
						ON did.id_client = cli.id_client
						ORDER BY razon_social ASC";
				$query = mysqli_query($con,$sql);
				$select = "<select name='cliente' style=\"width: 170px\" onchange='this.form.submit()'>";
				while($row = mysqli_fetch_row($query)) { 
					$select = $select.'<option value="'.$row[0].'"';
					if($row[1]==$_SESSION['cliente']) $select = $select." selected";
					$select = $select.'>'.$row[2]." ".$row[6].'</option>';
				}
				$texto = $texto . '</select>';
				echo $select; 
		 ...
		 <td>Fecha Desde:</td>
		 <td><input type="text" style="width: 50px" name="desde" id="desde" alt="fecha inicio" class="IP_calendar" title="Y-m-d" placeholder="YYYY-MM-DD" value="<?php echo $_SESSION['desde']; ?>"></td>
		 <td>Fecha Hasta:</td>
		 <td><input type="text" style="width: 50px" name="hasta" id="hasta" alt="fecha Fin" class="IP_calendar" title="Y-m-d" placeholder="YYYY-MM-DD" value="<?php echo $_SESSION['hasta']; ?>"></td>
  ...
  <script type="text/javascript">
	...
	if(isset($_POST['cliente']) & isset($_POST['desde']) & isset($_POST['hasta'])){
		...
		$query = "SELECT 
			   ...
			   cdr.billsec as Duracion,
			   cli.valor_min_pesos as Tarifa,
			   (cdr.billsec/60)*cli.valor_min_pesos as Valor,
			   cli.Origen as Tipo
		FROM tnagateway.cdr as cdr 
		FORCE INDEX(IDX_CallsDSTCallDate)
		INNER JOIN tnasolutions.did800 as cli
		ON cdr.dst = cli.did800
		where cdr.dst like '{$did800}' AND (call_start BETWEEN '{$desde}' AND '{$hasta}')
		;";
		...
		INNER JOIN tnasolutions.did800 as cli
		ON cdr.dst = cli.did800
		where cdr.dst like '{$did800}'
		&& cdr.calldate >= '".$desde." 00:00:00' 
		&& cdr.calldate <= '".$hasta." 23:59:59'
		;";
...
?> 		<div class="header">
		<div class="titulo" align="center" style="color: <?php echo $color; ?>">
			<?php echo "CDR de {$cliente}, RUT: {$rut} del DID {$did800}"; ?>
		</div>
		<div class="redondo">
		<table border="1">
			<tr align="center">
...
<div class="content"> <!-- comienzo div content con scroll-->
			...
			while($row = mysqli_fetch_row($result)) {
				...
				foreach($row as $cell) {
					...
					case 4: // Duración en min
						...
						break;
					case 5: // Valor en $
						$cell = number_format($duracion*$val_min, 0); ?>
						<td style='width: 10%' align='right'><?php echo $cell; ?></td>
						<?php $total += $cell;
						array_push($tmp,$cell);
						break;
		...
		$query = "SELECT count(cdr.dst) as total_llamadas,
						 sum(cdr.billsec/60) as total_minutos,
						 sum((cdr.billsec/60)*cli.valor_min_pesos) as total_pesos
				FROM tnagateway.cdr as cdr 
				FORCE INDEX(IDX_CallsDSTCallDate)
				INNER JOIN tnasolutions.did800 as cli
				ON cdr.dst = cli.did800
            ...
            while ($fila = mysqli_fetch_array($result)) {
				  $llamadas = $fila[0];
                  $consumo  = $fila[1];
                  $total    = $consumo * $val_min;
            }
			?> 
				<tr>
					<td colspan='8'><b><div align="center" style="align-content: center; text-align: center"> Valor Base de UF <?php echo $val_base; ?>, màs 
		...
		$tmp = "<script>
				...
				document.getElementById('baseuf').innerHTML = '".number_format($val_base, 2)."'";
