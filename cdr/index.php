...
<script type="text/javascript">
		function exportToExcel(tableId){
			...
			a.download = 'CDR' + getRandomNumbers() + '.xls'
			a.click()
		}
...
<body>
	...
	function primer_dia_mes_anterior() {
	  ...
	  return date('Y-m-d H:i:s', mktime(0,0,0, $month-1, 1, $year));
		...
		if($_SESSION['TNA']) {
		 ...
		 <?php }	?>
		 <td>           <input type="button" onClick="exportToExcel('tblToExcl')" value="Export to Excel" />
</td>	
	 </table>
  ...
  <script type="text/javascript">
	...
	if(isset($_POST['cliente']) & isset($_POST['desde']) & isset($_POST['hasta'])){
		...
		;"; 
    	// echo $query = "CALL consulta_cdr(".$id.")";       
		$result = mysqli_query($con,$query);
...
?> 		<div class="header">
		...
		<div class="redondo">			
			<table border="1" id="tblToExcl" name= "tblToExcl">
			<div id="invisible" class="logo">
				<tr>
					<td calspan="8" >
						<table  border="0" id="logo" name= "logo" hidden="">
							<tr>
								<td><img src="https://icontel.cl/wp-content/uploads/2023/06/Header-OK.jpg" alt="" width="380" hidden /></td>
							</tr>
						</table>
					</td>
				</tr>
				...
				<tr></tr>
				<tr>
					<td colspan="8" align="center" hidden ><strong><?php echo "CDR de {$cliente} "; ?><?php echo " RUT: {$rut}"; ?></strong></td>	
				</tr>	
			</div>	
			<tr style='color: black'>
				<!--th class='no-print' align="left" style="width: 2%">#</th-->									
				<th class="txtnegro" align="center" style="width: 5%">#</th>					
				<th class="txtnegro" style='width: 15%'>Fecha</th>
				<th class="txtnegro" style='width: 15%'>Origen</th>	
				<th class="txtnegro" style='width: 10%'>Destino</th>
				<th class="txtnegro" style='width: 10%'>Minutos</th>
				<th class="txtnegro" style='width: 10%x'>Valor $</th>
				<th class="txtnegro" style='width: 20%'>Tipo Llamada</th>
				<th class="txtnegro" style='width: 10%'>Tarifa $</th>
			</tr>
			<?PHP
			$consumo =0;
			$total =0;
			...
			while($row = mysqli_fetch_row($result)) {
				...
				$ptr = 0;
				$tmp = array();
				echo "<td align='center' style='width: 5%'>".$linea."</td>";	
				foreach($row as $cell) {
					$ptr ++;
					switch ($ptr) {
					case 1: // Fecha hora
